...
function service() {
  ...
  apps.use("/save_get", express.static(path.join(__dirname, "./n/d/")));

  apps.use(express.json());
  apps.post("/save", (req, res) => {
    res.set({ "Access-Control-Allow-Origin": "*" });
    try {
      let datas = req.body;
  ...
  apps.get("/get_save_data/:userId", (req, res) => {
    res.set({ "Access-Control-Allow-Origin": "*" });
  ...
  apps.get("/get", function (request, response) {
  ...
  apps.post("/", function (request, response) {
...
function App(){
  function createWindow() {
    const win = new BrowserWindow({
      width:  800,
      height:  600,
      frame: false, //  フレームを表示にする
      icon: path.join(__dirname, 'data/img/icon.ico'), //  アイコンを設定
      webPreferences: {
        nodeIntegration: true,
      }
    });
    //  タイトルロゴとタイトルテキストを設定
    win.setTitle('Mojimon Install Edition');
